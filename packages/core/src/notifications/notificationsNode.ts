...
export class NotificationsNode implements TreeNode {
    ...
    private readonly nodes: Record<string, TreeNode> = {}
    ...
    public getChildren() {
        const buildNode = (n: ToolkitNotification, type: NotificationType) => {
            return this.openNotificationCmd!.build(n).asTreeNode({
                ...
                contextValue: type === 'startUp' ? 'awsAmazonQWhatsNew' : 'awsAmazonQEmergency',
            })
        }
    ...
    // public dismissNotification(id: string /*node: TreeNode*/) {
    ...
    // public addNotification(notification: ToolkitNotification, type: NotificationType) {
    //     if (notification.id in this.nodes) {
    //         return
    //     }

    //     this.nodes[notification.id] = this.buildNode(notification, type)

    //     void setContext(this.showContextStr, true)
    //     this.refresh()
    //     if (type === 'emergency') {
    //         void vscode.commands.executeCommand(this.focusCmdStr)
    //     }
    // }
