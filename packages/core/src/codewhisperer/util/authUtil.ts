...
export class AuthUtil {
    ...
    async migrateExistingConnection(clientName: string) {
        ...
        let toImport: (SsoProfile & { id: string }) | undefined
        ...
        if (profiles) {
            ...
            if (toImport) {
                ...
                const pathed = (str: string) => {
                    ...
                    return path.join(getCacheDir(), hasher.update(str).digest('hex') + '.json')
                }
                ...
                const fromRegistrationFileName = getRegistrationCacheFile(getCacheDir(), {
                    startUrl: toImport.startUrl,
                    region: toImport.ssoRegion,
                    scopes: toImport.scopes,
                })
                const fromTokenFileName = getTokenCacheFile(getCacheDir(), toImport.id)

                await fs.rename(fromRegistrationFileName, toRegistrationFileName)
                await fs.rename(fromTokenFileName, toTokenFileName)
                await this.session.updateProfile({
                    startUrl: toImport.startUrl,
                    region: toImport.ssoRegion,
                    scopes: toImport.scopes ?? [],
