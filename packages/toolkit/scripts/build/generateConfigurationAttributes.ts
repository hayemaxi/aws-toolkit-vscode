...
const config = [
    {
        ...
        // writes to core-lib, because that is where the code that uses the defintions is
        ...
        headerComment: `
/**
 * This file is not generated from the core library. Please regenerate from 'packages/toolkit',
 * e.g. 'npm run generateConfigurationAttributes -w packages/toolkit'
 */
`,
    },
]
...
async function generateConfigurationAttributes(): Promise<void> {
    for (const debugConfiguration of packageJson.contributes.debuggers) {
        ...
        if (debuggerConfig.outputFolderExists && !fs.existsSync(path.basename(debuggerConfig.outputFile))) {
            throw new Error(`Output file parent ${debuggerConfig.outputFile} does not exist.`)
        }
